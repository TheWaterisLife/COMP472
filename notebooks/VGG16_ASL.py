...
Usage:  python notebooks/VGG16_ASL.py
"""
...
RESULTS_DIR = os.path.join(BASE_DIR, 'results', 'vgg16')
...
def build_vgg16(num_classes, pretrained=False):
    """Build VGG16 — from scratch or with ImageNet weights."""
    if pretrained:
        model = models.vgg16(weights=models.VGG16_Weights.DEFAULT)
        for param in model.features.parameters():
            param.requires_grad = False
    else:
        # Use vgg16_bn for from-scratch (has BatchNorm in backbone)
        model = models.vgg16_bn(weights=None)

    # Replace classifier head (with BatchNorm for stable gradients)
...
def save_model(model, class_names, save_name, arch='vgg16', num_classes=None):
...
if __name__ == '__main__':
    ...
    for ds_key in ['commands', 'digits', 'alphabets']:
        ds = DATASETS[ds_key]
        print(f"\n{'='*60}")
        print(f"  VGG16 — {ds['name']}")
        print(f"{'='*60}")

        train_loader, val_loader, test_loader, classes = create_loaders(ds['path'])

        model = build_vgg16(ds['num_classes'], pretrained=False)
        history = train_model(model, train_loader, val_loader)

        plot_history(history, f"VGG16 — {ds['name']}", f"{ds_key}")
        evaluate_model(model, test_loader, classes,
                       f"VGG16 — {ds['name']}", f"{ds_key}")
        save_model(model, classes, ds_key, arch='vgg16', num_classes=ds['num_classes'])
    ...
    model_tl = build_vgg16(ds['num_classes'], pretrained=True)
    history_tl = train_model(model_tl, train_loader, val_loader)

    plot_history(history_tl, f"VGG16 TL — {ds['name']}", "alphabets_tl")
    evaluate_model(model_tl, test_loader, classes,
                   f"VGG16 Transfer Learning — {ds['name']}", "alphabets_tl")
    save_model(model_tl, classes, 'alphabets_tl', arch='vgg16_tl', num_classes=ds['num_classes'])
